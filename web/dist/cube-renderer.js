...
function visibleStickers(state) {
    const visible = new Map();
    faceLayouts.forEach((face) => {
        face.stickers.forEach((index, position) => {
            const coordinates = faceCoordinates(face.name, Math.floor(position / 3), position % 3);
...
function createCubie(coordinates, stickers) {
...
export class CubeRenderer {
    cube;
    playbackSpeed;
    constructor(cube, playbackSpeed) {
        this.cube = cube;
        ...
        for (let x = -1; x <= 1; x += 1) {
            for (let y = -1; y <= 1; y += 1) {
                for (let z = -1; z <= 1; z += 1) {
                    if (x === 0 && y === 0 && z === 0)
                        continue;
                    fragment.append(createCubie({ x, y, z }, visible.get(`${x},${y},${z}`) ?? {}));
                }
            }
        }
